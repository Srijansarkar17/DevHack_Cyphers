...
def calculate_solar_potential(area: float, efficiency: float = 0.15, performance_ratio: float = 0.75, peak_sun_hours: float = 4) -> float:
    ...
    solar_potential = area * solar_irradiance * efficiency * performance_ratio * peak_sun_hours
    return solar_potential

def no_of_solar_panels(area: float):
    no_of_panels = int(area/18)
    return no_of_panels

def annual_savings(solar_potential: float, electricity_rate: float = 6) -> float:
    electricity = 5000
    monthly_consumption = electricity/electricity_rate
    monthly_energy_production = solar_potential

    savings = monthly_consumption - monthly_energy_production
    return savings
...
async def analyze_rooftop(file: UploadFile = File(...)):
    logger.info("Received request to /analyze_rooftop")
    try:
        ...
        panel_no = no_of_solar_panels(total_area)
        savings = annual_savings(solar_potential)
        logger.info(f"Calculated total area: {total_area}, solar potential: {solar_potential}, panel_no: {panel_no}, savings: {savings}")
